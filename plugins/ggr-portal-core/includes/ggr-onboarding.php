...
function ggr_onboarding_get_stages() {
    return array(
        ...
        'sign_contract'      => 'Inschrijfformulier tekenen',
        'validating'         => 'In behandeling',
        'extra_info'         => 'Aanvullende informatie aanleveren',
        'transfer_funds'     => 'Geld overmaken',
        'active_participant' => 'Actieve participant',
    );
}
...
function ggr_onboarding_get_status( $user_id ) {
    ...
    $legacy_status_map = array(
        'transfer_completed' => 'transfer_funds',
        'transfer_review'    => 'transfer_funds',        
    );
...
if ( ! function_exists( 'ggr_portal_get_payment_details' ) ) {
    function ggr_portal_get_payment_details() {
        $defaults = array(
            'iban'   => defined( 'GGR_PAYMENT_IBAN' ) ? GGR_PAYMENT_IBAN : '',
            'tenaam' => defined( 'GGR_PAYMENT_TENAAM' ) ? GGR_PAYMENT_TENAAM : '',
            'bank'   => '',
        );

        $payment_details = apply_filters( 'ggr_portal_investeren_payment_details', $defaults );
        return is_array( $payment_details ) ? $payment_details : array();
    }
}
...
function ggr_onboarding_build_review_sections( $user_id, $user, $participation_profile, $amount_display ) {
    ...
    $kyc_city          = get_user_meta( $user_id, 'ggr_kyc_city', true );
    if ( ! $kyc_city ) {
        $kyc_city = get_user_meta( $user_id, 'ggr_kyc_city_country', true );
    }
    $co_city = get_user_meta( $user_id, 'ggr_co_city', true );
    if ( ! $co_city ) {
        $co_city = get_user_meta( $user_id, 'ggr_co_city_country', true );
    }
    $kyc_nationality  = get_user_meta( $user_id, 'ggr_kyc_nationality', true );
...
<?php if ( 'application' === $type ) : ?>
	...
	$place_raw     = (string) get_user_meta( $user_id, 'ggr_contract_signed_place', true );
	if ( ! $place_raw ) {
		$place_raw = (string) get_user_meta( $user_id, 'ggr_kyc_city', true );
	}
	if ( ! $place_raw ) {
		$place_raw = (string) get_user_meta( $user_id, 'ggr_kyc_city_country', true );
	}
	if ( ! $place_raw ) {
	...
	<div class="page-content">
		...
		<div class="section">
			...
			<p class="muted">
			De ondertekende verklaart haar middelen te hebben vergaart middels; 
			</p>
			...
			<p style="margin-top: 2mm;"><strong>Toelichting</strong> (herkomst middelen)</p>
		...
		<div class="section">
			...
			<div class="signature-blocks <?php echo ( 'ja' === $has_co ) ? '' : 'signature-blocks--single'; ?>">
				...
				<div class="signature-box">
					...
					<div class="signature-line"></div>
					<div class="signature-name signature-manager">Wouter Ruiter</div>
				</div>				
			</div>
		</div>
...
function ggr_onboarding_dashboard_shortcode() {
    ...
    $extra_step_label     = $extra_step_label ? $extra_step_label : $extra_step_default_label;
    $extra_question_label = $extra_question_label ? $extra_question_label : $extra_step_label;
    $extra_upload_label   = $extra_upload_label ? $extra_upload_label : 'Upload aanvullende documentatie';
    ...
    if ( 'sign_contract' === $status && isset( $_POST['ggr_sign_contract_confirm'] ) ) {
        if ( ! isset( $_POST['ggr_sign_contract_nonce'] ) || ! wp_verify_nonce( $_POST['ggr_sign_contract_nonce'], 'ggr_sign_contract' ) ) {
            ...
            $signature_text = isset( $_POST['ggr_contract_signature_text'] )
                ...
                : '';
            $signature_place = isset( $_POST['ggr_contract_signed_place'] )
                ? sanitize_text_field( wp_unslash( $_POST['ggr_contract_signed_place'] ) )
                : '';                
            ...
            if ( '' === $signature_place ) {
                $messages['error'][] = 'Vul de plaats van ondertekening in.';
            }
    ...
    if ( in_array( $status, array( 'transfer_funds', 'transfer_completed' ), true ) && isset( $_POST['ggr_payment_confirm'] ) ) {
        if ( ! isset( $_POST['ggr_payment_confirm_nonce'] ) || ! wp_verify_nonce( $_POST['ggr_payment_confirm_nonce'], 'ggr_payment_confirm' ) ) {
            ...
            if ( function_exists( 'ggr_meldingen_add' ) ) {
                ggr_meldingen_add(
                    ...
                    $user_id,
                    array( 'onboarding_status' => 'transfer_funds' )
                );
            }
            ...
            $messages['success'] = 'We hebben je bevestiging ontvangen. We verwerken je betaling zo snel mogelijk.';
        }
    }
        ...
        if ( ! $extra_step_required ) {
            ...
            if ( is_wp_error( $result ) ) {
                ...
                if ( 'submit' === $extra_action ) {
                    ...
                    update_user_meta( $user_id, 'ggr_collecting_extra_required', 0 );                    
                    $status       = 'validating';
                    $status_label = isset( $stages[ $status ] ) ? $stages[ $status ] : $status;
 ...
 $amount_meta    = get_user_meta( $user_id, 'ggr_participation_amount', true );
    ...
    $existing_signature_place = get_user_meta( $user_id, 'ggr_contract_signed_place', true );    
            ...
            <div class="ggr-onboarding-body">
                ...
                <div class="ggr-onboarding-side">
                    <div class="ggr-onboarding-hero">
                                ...
                                <form method="post" class="ggr-onboarding-form" data-ggr-contract-signing>
                                    ...
                                    <div class="ggr-onboarding-field">
                                        ...
                                        <div class="ggr-onboarding-signature">
                                            ...
                                            <label class="ggr-onboarding-field-label" for="ggr_contract_signature_text">Getypte handtekening</label>
                                            ...
                                            <label class="ggr-onboarding-field-label" for="ggr_contract_signed_place">Plaats van ondertekening *</label>
                                            <input type="text" id="ggr_contract_signed_place" name="ggr_contract_signed_place" value="<?php echo esc_attr( $existing_signature_place ); ?>" required>                                            
                                    </div>
                                    <?php if ( 'ja' === $has_co_participant ) : ?>
                                        <div class="ggr-onboarding-field">
                                            <div class="ggr-onboarding-signature">
                                                ...
                                                <label class="ggr-onboarding-field-label" for="ggr_co_contract_signature_text">Getypte handtekening mede-participant</label>
                        ...
                        <?php elseif ( 'transfer_funds' === $status ) : ?>
                            <div class="ggr-onboarding-step-card">
                                <div class="ggr-onboarding-step-text">
                                    <h3 class="ggr-onboarding-step-heading">
                                        Rond de betaling af
                                    </h3>
                                   <p class="ggr-onboarding-step-description">Maak het overeengekomen bedrag van <strong><?php echo esc_html( $amount_display ); ?></strong> over naar onze rekening. Zodra de betaling is verwerkt, staat het bedrag definitief in je profiel.</p>
                                </div>
                                <div class="ggr-onboarding-alert">
                                    <p><strong>Betaalinstructie:</strong> Maak over naar
                                        ...
                                        Gebruik als kenmerk: <strong><?php echo esc_html( $payment_reference ); ?></strong>.
                                    </p>
                                ...
                                <form method="post" class="ggr-onboarding-form">
                                    <?php if ( 'transfer_funds' === $status && ! $payment_confirmation_label ) : ?>
                        ...
                        <?php if ( 'collecting' === $status ) : ?>
                            ...
                            <?php elseif ( 'personal' === $current_collecting_step ) : ?>
                                ...
                                <div class="ggr-onboarding-step-card">
                                    ...
                                    <form method="post" class="ggr-onboarding-form">
                                        ...
                                        $kyc_country        = get_user_meta( $user_id, 'ggr_kyc_country', true );
                                        if ( ! $kyc_country ) {
                                            $kyc_country = get_user_meta( $user_id, 'ggr_kyc_birth_country', true );
                                        }
                                        $kyc_birth_country  = get_user_meta( $user_id, 'ggr_kyc_birth_country', true );
                                        $kyc_birth_place    = get_user_meta( $user_id, 'ggr_kyc_birth_place', true );
                                        $kyc_nationality    = get_user_meta( $user_id, 'ggr_kyc_nationality', true );
                                        ...
                                        $co_country         = get_user_meta( $user_id, 'ggr_co_country', true );
                                        if ( ! $co_country ) {
                                            $co_country = get_user_meta( $user_id, 'ggr_co_birth_country', true );
                                        }
                                        ...
                                        if ( isset( $_POST['ggr_collecting_personal_submit'] ) ) {
                                            ...
                                            $kyc_city           = isset( $_POST['ggr_kyc_city'] ) ? sanitize_text_field( wp_unslash( $_POST['ggr_kyc_city'] ) ) : get_user_meta( $user_id, 'ggr_kyc_city', true );
                                            if ( ! $kyc_city ) {
                                                $kyc_city = get_user_meta( $user_id, 'ggr_kyc_city_country', true );
                                            }
                                            $kyc_country        = isset( $_POST['ggr_kyc_country'] ) ? sanitize_text_field( wp_unslash( $_POST['ggr_kyc_country'] ) ) : get_user_meta( $user_id, 'ggr_kyc_country', true );
                                            if ( ! $kyc_country ) {
                                                $kyc_country = get_user_meta( $user_id, 'ggr_kyc_birth_country', true );
                                            }
                                            ...
                                            $co_city         = isset( $_POST['ggr_co_city'] ) ? sanitize_text_field( wp_unslash( $_POST['ggr_co_city'] ) ) : $co_city;
                                            if ( ! $co_city ) {
                                                $co_city = get_user_meta( $user_id, 'ggr_co_city_country', true );
                                            }
                                            $co_country      = isset( $_POST['ggr_co_country'] ) ? sanitize_text_field( wp_unslash( $_POST['ggr_co_country'] ) ) : $co_country;
                                            if ( ! $co_country ) {
                                                $co_country = get_user_meta( $user_id, 'ggr_co_birth_country', true );
                                            }
                                    ...
                                    <?php
                                    $countries                = ggr_get_countries_nl();
                                    $selected_country         = $kyc_country ? $kyc_country : '';
                                    $selected_nationality     = $kyc_nationality ? $kyc_nationality : '';
                                    $selected_co_country      = $co_country ? $co_country : '';
                                    ...
                                    <div class="ggr-onboarding-grid ggr-onboarding-grid--columns-4">
                                        ...
                                        <div class="ggr-onboarding-field">
                                            ...
                                            <input type="text" id="ggr_kyc_city" name="ggr_kyc_city"
                                                   value="<?php echo esc_attr( $kyc_city ); ?>"
                                                   required>
                                        </div>
                                            <div class="ggr-onboarding-field">
                                            <label for="ggr_kyc_country">Land *</label>
                                            <select id="ggr_kyc_country" name="ggr_kyc_country" required>
                                    ...
                                    <div class="ggr-onboarding-section ggr-onboarding-section--co <?php echo $co_required ? '' : 'is-hidden'; ?>" data-co-section>
                                        ...
                                        <div class="ggr-onboarding-grid ggr-onboarding-grid--columns-3">
                                            ...
                                            <div class="ggr-onboarding-field">
                                                ...
                                                <input type="text" id="ggr_co_city" name="ggr_co_city"
                                                       value="<?php echo esc_attr( $co_city ); ?>" <?php echo $co_required ? 'required' : ''; ?>>
                                            </div>
                                            <div class="ggr-onboarding-field">
                                            <label for="ggr_co_country">Land *</label>
                                            <select id="ggr_co_country" name="ggr_co_country" <?php echo $co_required ? 'required' : ''; ?>>
                            ...
                            <?php elseif ( 'files' === $current_collecting_step ) : ?>
                                ...
                                <div class="ggr-onboarding-step-card">
                                    ...
                                    <div class="ggr-onboarding-file-grid">
                                        ...
                                        <div class="ggr-onboarding-field ggr-onboarding-field--file<?php echo ( 'zakelijk' === $participation_profile ) ? '' : ' is-hidden'; ?>" data-business-doc="true">
                                            <span class="ggr-onboarding-file-tag">Zakelijk</span>
                                            <label for="ggr_doc_share_register">Aandeelhoudersregister of overeenkomst</label>
...
function ggr_onboarding_handle_collecting_personal( $user_id ) {
    ...
    if ( isset( $field_values['ggr_kyc_city'] ) ) {
        update_user_meta( $user_id, 'address_city', $field_values['ggr_kyc_city'] );
    }
...
/**
 * Stap 6 collecting: aanvullende informatie en optionele upload.
 */
function ggr_onboarding_handle_collecting_extra( $user_id ) {

    if ( ! isset( $_POST['ggr_collecting_extra_nonce'] ) ||
         ! wp_verify_nonce( $_POST['ggr_collecting_extra_nonce'], 'ggr_collecting_extra' ) ) {
        return new WP_Error( 'invalid_nonce', 'Ongeldige sessie. Probeer het opnieuw.' );
    }
    ...
    $extra_required = (bool) get_user_meta( $user_id, 'ggr_collecting_extra_required', true );
    if ( ! $extra_required ) {
        return new WP_Error( 'extra_disabled', 'De aanvullende stap is niet actief.' );
    }

    $response = isset( $_POST['ggr_collecting_extra_response'] )
        ? sanitize_textarea_field( wp_unslash( $_POST['ggr_collecting_extra_response'] ) )
        : '';

    if ( '' === trim( $response ) ) {
        return new WP_Error( 'missing_extra_info', 'Vul de aanvullende informatie in.' );
    }
